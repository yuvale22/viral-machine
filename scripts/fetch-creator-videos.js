...
async function saveVideo(v, creator) {
  // Extract audio URL: prefer original music if available (smaller file), otherwise use video URL
  const useMusic = v.music_info?.original === true && (v.music || v.music_info?.play_url);
  const audioUrl = useMusic
    ? (v.music || v.music_info?.play_url)
    : (v.play || v.hdplay || v.video?.play_addr?.url_list?.[0] || '');
  ...
  const row = {
    ...
    he_subtitle: '',
    audio_url: audioUrl,
    audio_source: useMusic ? 'music_original' : 'video_extracted',
  };
  if (!row.video_id) return false;
  if (!row.audio_url) return false; // skip if no audio URL — can't be indexed later
